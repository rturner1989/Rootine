...
import type { JournalCalendarSummary, JournalKind } from '../../types/journal'
import Action from '../ui/Action'
import Heading from '../ui/Heading'
import { KIND_LABEL } from './filter/config'
import PlantThumb from './filter/PlantThumb'

const KIND_DOT: Record<JournalKind, string> = {
  water: 'bg-sky-deep',
  feed: 'bg-leaf',
  photo: 'bg-coral',
...
const KIND_ORDER: JournalKind[] = ['water', 'feed', 'photo', 'achievement', 'acquisition']

function RailBlock({ title, children }: { title: ReactNode; children: ReactNode }) {
  return (
    <div>
...
export type StatsRailProps = {
  summary?: JournalCalendarSummary
}
...
export default function StatsRail({ summary }: StatsRailProps) {
  const entryCount = summary?.entry_count ?? 0
  const kindCounts: Partial<Record<JournalKind, number>> = summary?.kind_counts ?? {}
