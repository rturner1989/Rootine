...
import type { JournalEntry, ScheduledCareItem } from '../../types/journal'
...
function groupByDay<Item>(items: Item[], keyOf: (item: Item) => string | null): Map<string, Item[]> {
...
export default function WeekAgenda({ weekStart, scheduled = [], filters, today, fill = false }: WeekAgendaProps) {
  ...
  const entriesByDay = groupByDay<JournalEntry>(data?.pages.flatMap((page) => page.entries) ?? [], (entry) =>
    entry.occurred_at ? isoDateKey(new Date(entry.occurred_at)) : null,
  )
  const scheduledByDay = groupByDay<ScheduledCareItem>(scheduled, (item) => item.date)
