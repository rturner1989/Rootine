...
import { type CalendarCell, type CareDotVariant, weekdayLabels } from '../../../utils/calendarGrid'
...
function hasVariant(cell: CalendarCell, variant: CareDotVariant): boolean {
...
type CellSummary = { text: string; tone: string }
...
function cellSummary(cell: CalendarCell): CellSummary | null {
...
function dayLabel(cell: CalendarCell): string {
  ...
  const byVariant = (variant: CareDotVariant) =>
    cell.dots.filter((dot) => dot.variant === variant).map((dot) => DOT_LABEL[dot.kind])
...
function renderDayNum(cell: CalendarCell) {
  return (
...
function renderDots(cell: CalendarCell) {
...
function renderSummary(cell: CalendarCell) {
...
export default function MonthGrid({
  ...
  const fallbackKey = monthDays.find((cell) => cell.isToday)?.key ?? monthDays[0]?.key ?? null
    ...
    return (
      <li key={cell.key}>
        <button
          ...
          // Only in-month cells reach this branch, and indexByKey is built
          // from exactly those cells, so the lookup always hits.
          onKeyDown={(event) => handleKeyDown(event, indexByKey.get(cell.key) as number)}
