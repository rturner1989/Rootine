...
import DayDetailSurface, { type DayDetailCloseInfo, type DaySelection } from './entries/DayDetailSurface'
import Toolbar from './entries/Toolbar'
import { type CalendarFilters, dateRangeSummaryLabel, readJournalFilters } from './filter/config'
...
export default function Entries({ plantId = null, fill = false }: EntriesProps) {
  ...
  function openDay(cell: CalendarCell, element: HTMLButtonElement) {
    ...
    // vertical/horizontal are each a two-member literal union, so the
    // template always produces one of PopoverPlacement's four members —
    // TS just doesn't fold the interpolation into that literal type itself.
    setSelected({ cell, placement: `${vertical}-${horizontal}` as DaySelection['placement'] })
  }

  function closeDay({ reason }: DayDetailCloseInfo = {}) {
