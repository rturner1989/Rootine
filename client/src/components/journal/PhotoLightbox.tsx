...
import type { PhotoFeedItem } from '../../types/plantPhoto'
import { formatLongDate } from '../../utils/dates'
import Action from '../ui/Action'

// Inherited from motion.img's own prop type rather than importing Motion's
// PanInfo type by name (framer-motion doesn't export it from a stable
// public path) — mirrors Dialog's onDragEnd typing.
type DragEndHandler = NonNullable<ComponentProps<typeof motion.img>['onDragEnd']>

const FOCUSABLE = 'button:not([disabled]), [href], [tabindex]:not([tabindex="-1"])'

// document.activeElement types as plain Element, which has no focus
// method — mirrors Dialog's isFocusableElement duck-type check.
...
export type PhotoLightboxProps = {
  photo: PhotoFeedItem | null
  onClose: () => void
  onDelete?: (photo: PhotoFeedItem) => void
  onPrev: () => void
  onNext: () => void
  hasPrev?: boolean
  hasNext?: boolean
...
export default function PhotoLightbox({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (target && isFocusableElement(target)) target.focus()
    }
  }, [open])
