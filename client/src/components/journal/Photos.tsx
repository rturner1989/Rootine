...
export default function Photos({ plantId = null, fill = false }: PhotosProps) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<PhotoFeedItem | null>(null)
  const activePhoto = activeIndex != null ? photos[activeIndex] : null
  const deletePhoto = useDeletePhoto()
  const { openPicker, isUploading } = usePhotoPicker(plantId ?? undefined)
  const toast = useToast()

  const closeLightbox = useCallback(() => setActiveIndex(null), [])
  const showPrev = useCallback(() => setActiveIndex((index) => (index != null && index > 0 ? index - 1 : index)), [])
      ...
      <>
        <div className="columns-2 sm:columns-3 lg:columns-4 gap-2">
          {photos.map((photo, index) => (
            ...
            >
              <img src={photo.image_url ?? undefined} alt="" loading="lazy" className="w-full block" />
            </Action>
          ))}
        </div>
