...
type NotificationPreferenceField = 'notify_care_reminders' | 'notify_achievements'

type PreferenceRow = {
  field: NotificationPreferenceField
  family: (typeof NOTIFICATION_FAMILIES)[keyof typeof NOTIFICATION_FAMILIES]
  title: string
  meta: string
}
...
export type NotificationsCardProps = {
  profile?: User
  onChange: (field: NotificationPreferenceField, value: boolean, label: string) => void
  disabled?: boolean
}

export default function NotificationsCard({ profile, onChange, disabled = false }: NotificationsCardProps) {
  const baseId = useId()
