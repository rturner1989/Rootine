...
describe('Medallion', () => {
  ...
  it('defaults to the brand scheme at md', () => {
    const { container } = render(<Medallion>R</Medallion>)
    const disc = container.firstChild as HTMLElement
  ...
  it('falls back to defaults for an unknown size or scheme', () => {
    const { container } = render(
      // @ts-expect-error — 'enormous'/'chartreuse' are deliberately outside
      // Medallion's size/scheme unions, proving unrecognised values fall
      // back to defaults.
