...
import type * as WeatherIconModule from '../../../../src/components/today/WeatherIcon'
import type { useWeather } from '../../../../src/hooks/useWeather'
...
const { week, todayKey } = vi.hoisted(() => {
  ...
  const days = Array.from({ length: 7 }, (_, offset) => {
    ...
    return { date: isoLocal(date), scheme: 'sun', icon: 'clear', icon_name: 'clear', label: 'Sunny', temperature: 20 }
  })
  return { week: days, todayKey: isoLocal(start) }
})

// WeekStrip only reads `week` off useWeather's return value.
vi.mock('../../../../src/hooks/useWeather', (): { useWeather: () => Pick<ReturnType<typeof useWeather>, 'week'> } => ({
  useWeather: () => ({ week }),
}))
vi.mock('../../../../src/components/today/WeatherIcon', (): typeof WeatherIconModule => ({
  default: () => <span data-testid="wx" />,
}))
