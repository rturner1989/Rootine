...
import type { Space, SpacePreset } from '../../../src/types/space'
...
// rest the way Space#create's defaults would. Every call site below
// supplies id/name/icon/category/archived_at on top of these defaults,
// which is what makes the `as Space` at the end a real (if unenforced)
// guarantee rather than a loosening.
function fullSpace(partial: Partial<Space>): Space {
  return {
    light_level: 'medium',
...
let createdSpaces: Space[] = []
let archivedSpaceIds: number[] = []
let nextSpaceId = 100
...
function renderWizard(props: Partial<{ open: boolean; onClose: () => void }> = {}) {
